...
$LogPath = Join-Path ([System.IO.Path]::GetTempPath()) "codexmate-tray-${pid}.log"
try {
...
public class CodexMateTrayIcon : IDisposable {
    ...
    public CodexMateTrayIcon(string tooltip, string url, string exitMarkerPath) {
        ...
        menu.Items.Add("Open").Click += (s, e) => { try { System.Diagnostics.Process.Start(url); } catch {} };
        menu.Items.Add("-");
...
'@
    ...
    $app = New-Object CodexMateTrayIcon -ArgumentList "${name}", "${url}", "${exitMarkerPath.replace(/\\/g, '/')}"
